...
For ganesha 9.6 + Debian trixie (trixie-backports) the generated CLIENT blocks now include an explicit `Read_Access_Check_Policy = pre;` (plus the same in EXPORT_DEFAULTS) and a tiny nfsidmap shim is used (only for ganesha) so the server does the same principal-to-uid lookups clients expect (`getent passwd testuser1` + full `user@REALM` / `host/...` via idhelper). The idhelper now uses structured LDAP (IdLdapResolver + caches) for its LDAP fallback, matching the UI's LdapClient.
