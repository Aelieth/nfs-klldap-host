...
| EXPORT Disable_ACL / Manage_Gids=false (NOACL 0.9.40-style path); distinct from ACL path; no post/Read/Enable/POSIX markers | `nfs-klldap-config/src/posix_only_policy.rs` (warnings), `src/generate.rs` (two paths), `tests/limited_fs_generate.rs`, `tests/cli_generate_gate.rs`, `tests/ganesha_96_identity_audit.rs` |
