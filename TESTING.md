...
`scripts/fedora-krb5p-client-validate.sh` — machine `kinit -k` + `sec=krb5p` mount/IO (host bind visibility). User TGT phase needs a Kerberos-synced principal, `--privileged --ipc=host`, host `rpc_pipefs` bind, client `rpc.idmapd` + passwd/group stubs, `[gssd] use-machine-creds=0`, and `${exp_uid}:${exp_gid}` on both client `stat` and hostdata bind. Server must emit `Only_Numeric_Owners=true`, `UseGetpwnam=true`, `Manage_Gids=false`, and preload `libnfs-klldap-createmode.so` (openat64 + getgrouplist) for Ganesha. Full gate: `SCRATCH=/path ./scripts/capture-plan-gate.sh` → `plan-gate.log` + `nfs-run-start-full.log`. Use `DOCKER_NO_CACHE=true` on `make docker` after generator changes.
