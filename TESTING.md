...
`scripts/fedora-krb5p-client-validate.sh` — machine `kinit -k` + `sec=krb5p` mount/IO (host bind visibility). User TGT phase needs a Kerberos-synced principal, `--privileged --ipc=host`, host `rpc_pipefs` bind, client `rpc.idmapd` + passwd/group stubs, `[gssd] use-machine-creds=0`, and `${exp_uid}:${exp_gid}` on both client `stat` and hostdata bind. Server emits Only_Numeric_Owners; UseGetpwnam=false for principal2grp/allocate_by_principal on user TGT + Manage_Gids; idhelper/extrausers for groups; preload. Full gate: `SCRATCH=/path ./scripts/capture-plan-gate.sh` → `plan-gate.log` + `nfs-run-start-full.log`. Use `DOCKER_NO_CACHE=true` on `make docker` after generator changes.
