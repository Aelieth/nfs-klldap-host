...
| Client share manifest `GET /client-manifest.json` (no session required; JSON + `no-store`; minimal fields — internal path keys never serialized; capable auto share publishes `acl`/"auto (on)", incapable publishes `noacl`/"auto (off)"; per-share `navahi` = effective exposure (global && flag, chip rule) so the client kit can cross-check avahi-browse against the flagged set; bypasses the setup-wizard redirect) | `nfs-klldap-ui/src/web/mod.rs` (`client_manifest_is_public_and_minimal`, `client_manifest_acl_share_reports_acl`, `client_manifest_bypasses_setup_gate`, `client_manifest_navahi_tracks_effective_exposure`) |
