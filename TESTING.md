...
| ACL editor gate = share serve-root mode ∧ node mount (explicit-off/auto-off/incapable block; auto+capable & explicit-on editable; explicit-on+Inconclusive editable with amber "on (unverified)"; capable submount under an ACL share allowed, incapable/unverified submount blocked) | `nfs-klldap-ui/src/web/permission_tree.rs` (`acl_capability_tests`), `web/mod.rs` (`web_acl_apply_refused_on_incapable_mount_422`) |
