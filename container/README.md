...
NSS: `nsswitch.conf` is `files extrausers sss`. idhelper materializes `/var/lib/extrausers/{passwd,group}` (no `#` lines or `:` in gecos — libnss-extrausers rejects both). Numeric uid/gid strings are never cached as Kerberos principals (prevents `getpwuid(3002)` returning login `3002`). LDAP primary groups include member logins for `group-test:x:3005:user,...`. Ganesha uses `LD_PRELOAD=libnfs-klldap-createmode.so:nss_wrapper` (strips FSAL `RTLD_DEEPBIND`, openat64/fchmod execute bits, getgrouplist shim), `UseGetpwnam=true`, `Only_Numeric_Owners=true` + `Allow_Numeric_Owners=true`, and krb5p `Manage_Gids=false`.
