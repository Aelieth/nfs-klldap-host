...
NSS: `nsswitch.conf` is `files extrausers sss`. idhelper materializes `/var/lib/extrausers/{passwd,group}` (no `#` lines or `:` in gecos — libnss-extrausers rejects both). Numeric uid/gid strings are never cached as Kerberos principals (prevents `getpwuid(3002)` returning login `3002`). LDAP primary groups include member logins for `group-test:x:3005:user,...`. Ganesha uses LD_PRELOAD=libnfs-klldap-createmode.so:nss_wrapper (getgrouplist shim + bits), UseGetpwnam=false (principal path for allocate_by_principal on user TGT), Only_Numeric, and krb5p Manage_Gids=true (uid2grp via principal2grp + idhelper).
