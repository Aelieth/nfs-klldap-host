...
NSS: `nsswitch.conf` is `files extrausers sss`. idhelper materializes `/var/lib/extrausers/{passwd,group}` and nss_wrapper files including `host/<client>@REALM` machine entries (uid/gid 0). Ganesha starts after idhelper socket is ready, under nss_wrapper LD_PRELOAD. UseGetpwnam=true routes Manage_Gids through uid2grp_allocate_by_uid + getgrouplist (nfs_creds.c / uid2grp.c pwutils path); machine principals return gid 0 (not 65534). Limited FS fragments emit `POSIX_ONLY_EXPORT` before `SecType` (`Disable_ACL` + `Read_Access_Check_Policy=post`). Preflight exercises user + server host + client host via GRPS, socket, nfsidmap `-g`, and ganesha-ctl; duplicate OK logs are deduped.
