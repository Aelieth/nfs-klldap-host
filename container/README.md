...
NSS: `nsswitch.conf` is `files extrausers sss`. idhelper materializes `/var/lib/extrausers/{passwd,group}` (no `#` lines or `:` in gecos — libnss-extrausers rejects both). Numeric uid/gid strings are never cached as Kerberos principals (prevents `getpwuid(3002)` returning login `3002`). LDAP primary groups include member logins for `group-test:x:3005:user,...`. Ganesha uses nss_wrapper LD_PRELOAD (when enabled), UseGetpwnam=false for uid2grp_allocate_by_principal on user TGTs with Manage_Gids=true, Only_Numeric, idhelper/extrausers for groups (machine 0 + LDAP users). Limited FS (noacl) fragments now include `Read_Access_Check_Policy="post"` + posix-only; runtime idhelper supplies groups for ls (krb5p noacl) via ID_MAPPER; lightweight check runs post-generate.
