...
The server must perform the same lookups clients do (`getent passwd testuser1` + principal forms). For ganesha 9.6 on Debian trixie, `principal2uid` calls in-process libnfsidmap (`nfs4_gss_princ_to_ids`), which does `getpwnam` inside ganesha.nfsd under nss_wrapper — so all LDAP users must be bulk-seeded into `/var/lib/nfs-klldap/nss_passwd` at idhelper startup (before Ganesha). The nfsidmap binary shim does not intercept that path. Explicit `Read_Access_Check_Policy = pre;` inside CLIENT blocks addresses ACCESS timing for krb5 compounds. Machine principals map to 0; some group-fetch INFO for uid 0 and winbind noise are expected. The idhelper (IdLdapResolver + bulk seed + getent) is authoritative.
