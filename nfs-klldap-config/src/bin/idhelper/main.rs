...
//! nfs-klldap-idhelper Central fast resolver for ganesha 9.6 .
...
mod tests {
    ...
    fn materialize_writes_machine_as_root() {
        ...
        // Temporarily override the const paths by using a temp dir and
        // monkey-patch via env is hard;
        // instead directly test the line builders and a small manual cache
        // write.
        let mut c = IdCache::default();
    ...
    // --- Additional repros from the exact full trace the user provided after
    // rebuild ---
    #[test]
    ...
    fn extract_only_good_from_full_clid_create_line() {
        // fs_create trace line must yield only the real host name.
        let line = r#"fs_create_clid_name :CLIENT ID :DEBUG :Created client name [::ffff:10.10.10.83-(21:Linux NFSv4.2 blue-lt)]"#;
        let r = extract_candidate_principal(line, "SATOMLIN.COM");
        if let Some(c) = r {
            assert!(c.contains("blue-lt"));
    ...
    fn stress_extract_on_trace_fragments_no_garbage() {
        ...
        let fragments = vec![
            ...
            // A line that contains nfsv4 early and later (nil) groups with no
            // good Linux group after the marker (to hit fallback)
    ...
    fn machine_principal_short_circuits_to_zero_without_getent() {
        ...
        // Synthetic / internal form should also short-circuit to 0:0
    ...
    fn extract_catches_could_not_map_line_for_user_principal() {
        ...
        // Ganesha logs this when its principal2uid can't map during
        // nfs_req_creds/ACCESS.
    ...
    fn extract_catches_get_uid_using_nfsidmap_line() {
        // Early sighting: Ganesha announcing it is calling the mapper for a
        // user principal.
        // Extracting here allows the observer to resolve *before or during*
        // the blocking shim call.
