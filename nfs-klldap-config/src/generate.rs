...
access_provider = {access}"#,
    ...
    // krb5_realm: always the effective realm .
    // No separate override field needed in this design .
    out.push_str(&format!("\nkrb5_realm = {}", realm));
...
fn write_ganesha_main(
    ...
    // Explicit %include per share for deterministic load order .
...
/// Build Ganesha 9.6 EXPORT ACL lines and auto-detect comment from probe
/// results.
...
mod tests {
    ...
    fn per_share_export_respects_rw_and_security_overrides() {
        // Exercise RO + krb5i; EXPORT must reflect them.
        let mut cfg = crate::NfsKlldapConfig {
            ldap_uri: "ldaps://k.test:6360".into(),
            sssd: crate::SssdSection {
                ldap_default_bind_dn: "uid=admin,ou=people,dc=test,dc=com".into(),
