...
/// Basename for a managed export fragment (matches write_export_fragme...
...
pub fn generate_all(cfg: &NfsKlldapConfig, paths: &GenerationPaths) -> Result<(), ConfigError> {
    ...
    // Write fragments before main conf so %include targets exist at parse t...
...
fn write_sssd_conf(cfg: &NfsKlldapConfig, out: &Path) -> Result<(), ConfigError> {
    ...
    // Edit nfs-klldap.conf; watcher/SIGHUP regenerates sssd.conf.
    let header = format!(
...
access_provider = {access}"#,
    ...
    // krb5_realm/server/kpasswd: from effective_realm + ldap_uri host unles...
    ...
    // krb5_realm: always the effective realm (same as kerberos.realm and kr...
    // No separate override field needed in this design (realm is single sou...
    out.push_str(&format!("\nkrb5_realm = {}", realm));
...
fn write_idmap_conf(cfg: &NfsKlldapConfig, out: &Path) -> Result<(), ConfigError> {
    ...
    let content = format!(
        r#"# GENERATED by nfs-klldap-config — do not edit (source: nfs-klldap.conf).
# libnfsidmap Domain/Local-Realms for Ganesha 9.6 principal2uid
...
fn write_ganesha_main(
    ...
    // Explicit %include per share for deterministic load order (no glob rac...
...
"#,
    ...
    // Baseline LOG for operators/idhelper; GANESHA_DEBUG enables FULL_DEBUG.
...
/// Build Ganesha 9.6 EXPORT ACL lines and auto-detect comment from pro...
...
fn write_export_fragments(cfg: &NfsKlldapConfig, exports_dir: &Path) -> Result<(), ConfigError> {
    ...
    for (i, share) in cfg.shares.iter().enumerate() {
        ...
        // Defensive absolutize; validate_and_derive normally guarantees absolut...
