...
fn write_sssd_conf(cfg: &NfsKlldapConfig, out: &Path) -> Result<(), ConfigError> {
    ...
    // Generated sssd.conf: edit nfs-klldap.conf; watcher/SIGHUP regenerates this file.
    let header = format!(
...
/// Write /etc/nfs.conf: prefer user krb5 creds for NFSv4 krb5p clients (not machine keytab only).
...
// Ganesha 9.6 reads pwnam via nsswitch; idhelper materializes principal→uid for hybrid clients.
