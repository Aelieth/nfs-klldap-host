...
access_provider = {access}"#,
    ...
    // krb5_realm/server/kpasswd: from effective_realm + ldap_uri host unless overridden.
...
/// Write /etc/idmapd.conf: Domain from effective_realm(), Method/GSS = nsswitch.
fn write_idmap_conf(cfg: &NfsKlldapConfig, out: &Path) -> Result<(), ConfigError> {
    let realm = cfg.nfsv4_domain();

    let content = format!(
        r#"# GENERATED by nfs-klldap-config — do not edit (source: nfs-klldap.conf).
# libnfsidmap Domain/Local-Realms for Ganesha 9.6 principal2uid; Method=nsswitch.
...
// Ganesha 9.6 pwnam=nsswitch; idhelper materializes principals for hybrid Kerberos.
...
fn write_export_fragments(cfg: &NfsKlldapConfig, exports_dir: &Path) -> Result<(), ConfigError> {
    ...
    for (i, share) in cfg.shares.iter().enumerate() {
        ...
        // Defensive absolutize; validate_and_derive normally guarantees absolute Pseudo.
        ...
        // CLIENT block: Protocols=4 only. Read_Access_Check_Policy omitted (9.6 rejects it).
...
mod tests {
    ...
    fn idmap_conf_generated_with_domain_from_realm_and_nsswitch() {
        ...
        // Read_Access_Check_Policy rejected by trixie 9.6; built-in default is pre.
        assert!(
