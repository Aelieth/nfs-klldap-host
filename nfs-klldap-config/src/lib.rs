...
// !nfs-klldap-config: TOML validation
...
/// Serializes env-mutating tests across modules (`cargo test --workspa...
#[cfg(test)]
...
mod tests {
    ...
    fn generate_produces_expected_artifacts() {
        ...
        // The lighter components we care about for principal discovery should b...
    ...
    fn ganesha_debug_log_block_emitted_only_when_env_true() {
        ...
        // always present now. Only the FULL_DEBUG variant is controlled by the...
        let cfg = minimal_cfg();
        let tmp = tempfile::tempdir().unwrap();
        let paths = GenerationPaths {
            sssd_conf: tmp.path().join("sssd.conf"),
        ...
        // 2) With GANESHA_DEBUG=true (any common truthy spelling) -> FULL_DEBUG...
