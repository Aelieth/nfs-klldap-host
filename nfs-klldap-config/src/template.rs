...
# Edit to change group for WebUI admins

[server]
# hostname = "myhost.example.com"                               # Default
# Optional override for keytab only. Recommended: docker run --uts=host
...
# ldap_user_search_base = "ou=people,dc=example,dc=com"          # Optional
# defaults to dc=<realm> (Subtree)
# ldap_group_search_base = "ou=people,dc=example,dc=com"        # Optional
# defaults to dc=<realm> (Subtree)
kllldap_ignored_attributes = true                               # KLLDAP specific - improves lookup time, prevents attribute spam

# ldap_tls_reqcert = "never"                                    # auto-derived
# typical for internal/self-signed
# ldap_tls_cacert = "/path/to/ca.pem"
# Optional custom CA (instead of never)
# ldap_id_use_start_tls = true
# Only for ldap:// with STARTTLS (not ldaps://)

[kerberos]
# realm = "EXAMPLE.COM"                                         # Default
# auto-derived from ldap_uri host, edit to override
...
[webui]
# tls = false
# Off via NFS_KLLDAP_WEBUI_TLS (reverse proxy)
# tls_cert = "/config/webui.crt"
# Custom cert path (NFS_KLLDAP_WEBUI_TLS_CERT wins)
# tls_key = "/config/webui.key"
# Custom key path (NFS_KLLDAP_WEBUI_TLS_KEY wins; 0600)
