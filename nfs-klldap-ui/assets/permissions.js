// ===== Detached "Share Permissions" panel behaviour (replaces inline dir-meta/editor/acl) =====
// Shared on window so index.html's page script and the apply poller can coordinate lock state.
window.applyInProgress = false;
...
window.permIsLocked = () => isEditing() || window.applyInProgress;

// Unsaved-edit tracking: set while editing on any panel input/change; gates Cancel.
let panelDirty = false;
window.permFlashLock = function () {
...
window.loadDirPerms = function (path) {
    if (!panel()) return;
    ...
    htmx.ajax('GET', '/dir-perms?path=' + encodeURIComponent(path),
        { target: '#perm-panel .perm-body', swap: 'innerHTML' }).then(() => {
            const pl = panel();
            pl.querySelector('.perm-share').textContent = window.currentShareName || '';
...
window.cancelCurrentApply = function() {
...
function aclApply(fields) {
    const pl = panel(); if (!pl) return;
    const path = pl.querySelector('.perm-path').textContent;
    const fd = new URLSearchParams(Object.assign({ path }, fields));
    fetch('/acl-apply', { method: 'POST', credentials: 'include',
        ...
        .then(res => {
            ...
            window.loadDirPerms(path);
        })
        .catch(() => {
            pl.querySelectorAll('.acl-del:disabled,.acl-add-btn:disabled')
                .forEach(b => b.disabled = false);
            showPermPanelError('ACL apply failed — the server rejected the request. See server logs.');
        });
}
...
document.addEventListener('click', function (e) {
    ...
    if (caret) {
        if (window.permIsLocked()) { window.permFlashLock(); e.stopPropagation(); return; }
        ...
        if (caret.getAttribute('aria-expanded') === 'true') {
            ...
            if (sib.dataset.loaded !== 'true') {
                ...
                caret.textContent = '⋯';
                htmx.ajax('GET', '/fs/children?path=' + encodeURIComponent(dir.dataset.path),
    ...
    if (dir && dir.dataset.path) {
        ...
        if (!dir.classList.contains('selected')) {
            ...
            window.loadDirPerms(dir.dataset.path);
        }
        return;
    }
    ...
    if (e.target.closest('.btn-cancel')) {
        if (panelDirty && !confirm('Discard unsaved permission changes?')) return;
        panelDirty = false;
        const path = pl.querySelector('.perm-path').textContent;
        if (path) window.loadDirPerms(path);
        return;
    }
    if (e.target.closest('.btn-apply')) {
        ...
        if (rec && rec.checked) {
            const path = pl.querySelector('.perm-path').textContent;
            if (!confirm('Recursively apply owner and mode to EVERYTHING under\n' + path + ' ?')) return;
        }
        panelDirty = false;
        window.applyInProgress = true;
...
document.addEventListener('htmx:afterSwap', function (evt) {
    ...
    if (t.querySelector('[data-applying]')) {
        ...
        setTreeLock(true);
        startApplyProgressPoller();
    } else if (window.applyInProgress) {
...
document.addEventListener('htmx:afterOnLoad', function (evt) {
    ...
    if (!window.applyInProgress && !ph) return;
    window.applyInProgress = false;
    const pathEl = panel() && panel().querySelector('.perm-path');
    const path = ph ? ph.dataset.path : (pathEl ? pathEl.textContent : '');
    setTreeLock(false);
    if (path) window.loadDirPerms(path);
});
...
function handleApplyRequestError(evt) {
    ...
    setPanelLoading(false);
    if (window.applyInProgress || isPoller) {
        stopApplyProgressPoller();
        window.applyInProgress = false;
        setPanelMode(false);
